...
module.exports = async (req, res) => {
  ...
  const INATIVOS=["reproved","disqualified","withdrawn","canceled","cancelled","rejected","declined","hired_other","desist"];
  const incluirInativos=String((req.query&&req.query.incluirInativos)||"")==="1";
  let totalApi=0, inativas=0, duplicadas=0;
  try{
    for(const jobId of ids){
      for(let page=1; page<=10; page++){
        ...
        for(const a of lista){
          ...
          const chave=String(a.id||"")+"|"+String((a.candidate&&a.candidate.email)||a.email||"");
    ...
    res.status(200).json({ok:true,total:candidatos.length,totalApi,inativas,duplicadas,candidatos,avisos});
  }catch(e){
